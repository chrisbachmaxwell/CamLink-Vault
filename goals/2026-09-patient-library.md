...
- [ ] Patients page rests as search + Browse; typing narrows; no
      folder-orphans, no Unfiled section (verified by: ui-gate asserts
      hidden-at-rest, filter-to-row flow, #unfiled-block absent)
- [ ] Ending a 0-photo visit saves nothing; /api/sessions never lists
      0-photo visits (verified by: ui-gate ends an empty visit and
      asserts it is absent; done card says "nothing was saved")
- [ ] Gallery opens from live grid and patient page, Esc returns,
      Download links wired (verified by: ui-gate)
- [ ] Visit zip endpoint streams a valid zip (verified by: ui-gate
      fetches it and checks the PK header)
- [ ] All six gates green; field: Chris browses, opens the gallery,
      downloads a visit (verified by: Chris's session)
